from utils.db import Fetch, commit, join

class CategoryModel:
    __where_id = ' WHERE categoryid = %s'
    __select_all = 'SELECT * FROM category'

    @staticmethod
    def get_categories():
        # Fetch all categories
        return Fetch.all('SELECT * FROM category ORDER BY categoryname')
    
    @classmethod
    def get_category(cls, categoryid):
        return Fetch.one(join(cls.__select_all, cls.__where_id), (categoryid,))

    @classmethod
    def get_category_by_name(cls, name):
        return Fetch.one(
            join(cls.__select_all, 'WHERE categoryname = %s'), 
            (name,)
        )
    
    @staticmethod
    def add_category(name, cat_type):
        return commit("""
                         INSERT INTO category (categoryname, type_)
                         VALUES (%s, %s)
                         """, (name, cat_type))
    
    @classmethod
    def edit_category(cls, id, name, cat_type):
        update = 'UPDATE category'
        return commit(
            join(update, 'SET categoryname = %s', cls.__where_id), 
            (name, id),
            join(update, 'SET type_ = %s', cls.__where_id), 
            (cat_type, id)
        )
    
    @classmethod
    def delete(cls, id):
        return commit(join('DELETE FROM category', cls.__where_id), 
                         (id,), 
                         return_was_affected=True,
                         return_id=False)