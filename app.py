...
@app.route('/')
def dashboard():
    """Main dashboard showing account balances and recent transactions"""
    try:
        ...
        print("err:", e)
        return render_template('dashboard.html', accounts=[], recent_transactions=[])
...
def add_transaction():
    ...
    if request.method == 'POST':
        ...
        try:
            accounts = get_accounts()
            categories = get_categories()
            return render_template('add_transaction.html', accounts=accounts, categories=categories, datetime=datetime)
        except Exception as e:
...
def edit_transaction():
    try:
        ...
        if request.method == 'POST':
            ...
            """, [transaction_id])
            # print(transaction)
            return render_template('edit_transaction.html', transaction=transaction, datetime=datetime, accounts=accounts, categories=categories)
    except Exception as e:
        flash('Error editing transaction', 'error')
        print("err:", e)
        return render_template('edit_transaction.html', transaction=None, datetime=datetime, accounts=[], categories=[])
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            for budget in budgets:
                ...
                actual = cursor.fetchone()['actual'] # execute() returns dict with only key 'actual'
                budget['actual'] = actual
                budget['remaining'] = budget['budget_amount'] - actual
        
        return render_template('budgets.html', budgets=budgets, year=year, month=month, datetime=datetime)
    except Exception as e:
        flash('Error loading budgets', 'error')
        print("err:", e)
        return render_template('budgets.html', budgets=[], year=datetime.now().year, month=datetime.now().month, datetime=datetime)
...
def add_budget():
    ...
    if request.method == 'POST':
        try:
            ...
            db_commit(""" 
                INSERT INTO budget (categoryid, budget_year, budget_month, budget_amount) 
                VALUES (%s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE budget_amount = %s
            """, (category_id, budget_year, budget_month, budget_amount, budget_amount))
            
            flash('Budget saved successfully!', 'success')
            return redirect(url_for('budgets', year=budget_year, month=budget_month))
        except Exception as e:
            flash('Error saving budget', 'error')
            print("err:", e)
            return render_template('add_budget.html', categories=[], datetime=datetime)
    
    else:
        try:
            categories = get_categories()
            return render_template('add_budget.html', categories=categories, datetime=datetime)
        except Exception as e:
            flash('Error loading categories', 'error')
            print("err:", e)
            return render_template('add_budget.html', categories=[], datetime=datetime)
...
def add_cashflow():
    ...
    try:
        if request.method == 'POST':
            ...
            """)
            return render_template('add_cashflow.html', transactions=transactions, cashflow_types=types)
        
    except Exception as e:
        flash('Error adding cashflow', 'error')
        print("err:", e)
        return render_template('add_cashflow.html', transactions=[], cashflow_types=types)
